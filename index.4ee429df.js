...
function changeTheme() {
    if (document.body.classList.value == "light") document.body.classList.value = "dark";
    else document.body.classList.value = "light";
...
for(let i2 = 0; i2 < topnavLinks.length; i2++)topnavLinks[i2].onclick = function() {
    topnavDef();
    goLocation(this);
};
...
window.onscroll = function() {
    if (popupForm.classList.value != "contactForm active") topnav.classList.toggle("sticky", window.scrollY > 68);
};
//settings functions
...
for(let i4 = 0; i4 < minPlus.length; i4++)minPlus[i4].onclick = function() {
    let op = minPlus[i4].getAttribute("data-op");
    let num = passLength.textContent;
    let result = parseInt(num) + parseInt(op);
    if (result >= 6 && result <= 99) passLength.textContent = result;
};
//footer navigation
for(let i5 = 0; i5 < footerNavLinks.length; i5++)footerNavLinks[i5].onclick = function() {
    goLocation(this);
};
//generator
let password = "";
...
},{}]},["1XQsC","SXDIM"], "SXDIM", "parcelRequire923e")
